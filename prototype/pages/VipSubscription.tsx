...
import { Check, Crown, Star, Shield, Zap, ChevronRight, X } from 'lucide-react';
...
const VipSubscription: React.FC = () => {
  ...
  const plans = [
    { id: 'month_trial', name: '首月体验转会', price: 9.9, originalPrice: 19.9, label: '新人特惠', period: '/首月', desc: '试训期，低成本体验' },
    { id: 'season', name: '主力赛季卡', price: 49.0, originalPrice: 59.7, label: '超值推荐', period: '/季', desc: '完美覆盖春/秋季联赛' },
    { id: 'year', name: '豪门终身约', price: 199.0, originalPrice: 238.8, label: '省¥40', period: '/年', desc: '稳定建制球队首选' },
  ];
    ...
    <div className="min-h-screen bg-[#1a1a1a] text-white pb-24 relative overflow-hidden">
        ...
        <div className="space-y-3 mb-8">
            {plans.map(plan => (
                <div 
                    ...
                    className={`relative rounded-xl p-1 transition-all duration-200 cursor-pointer ${selectedPlan === plan.id ? 'bg-gradient-to-r from-[#07c160] to-emerald-500 shadow-lg shadow-green-900/50' : 'bg-white/5 hover:bg-white/10'}`}
                >
                    <div className="bg-[#252525] rounded-[10px] p-4 flex justify-between items-center h-full">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-[#1a1a1a] border-t border-white/10 z-20 max-w-md mx-auto">
          <button 
            ...
            className="w-full bg-gradient-to-r from-[#07c160] to-emerald-600 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-green-900/30 active:scale-[0.98] transition-transform flex items-center justify-center gap-2"
          >
