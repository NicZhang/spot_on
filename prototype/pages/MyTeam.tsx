...
import { Users, Wallet, ChevronRight, BellRing, Settings, Plus, Trash2, ArrowUpCircle, ArrowDownCircle, X, ArrowLeftRight } from 'lucide-react';
        ...
        {activeTab === 'members' ? (
               ...
               >
                 ...
                 <div className="flex items-center gap-3">
                    <div className="text-right">
                     ...
                     {/* Remove Action (Mock: don't allow removing self) */}
                    {player.id !== 'p1' && (
                        <button 
                            onClick={(e) => { 
                                e.stopPropagation();
                                if(confirm(`确定要移除 ${player.name} 吗？`)) removePlayer(player.id); 
                            }}
          ...
          <div className="space-y-4">
            ...
            {financeSubTab === 'bills' ? (
                /* Bill List */
                <div className="space-y-3">
                    {bills.length === 0 && <div className="text-center text-gray-400 text-sm py-8">暂无待收账单</div>}
                    {bills.map(bill => (
          ...
          <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center sm:p-4" onClick={() => setShowInviteModal(false)}>
              <div className="bg-white w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl p-6 animate-slide-up" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                      <button 
                        onClick={() => { alert('模拟：已唤起微信分享，好友点击后将进入邀请落地页'); setShowInviteModal(false); }}
                      ...
                      <button 
                        onClick={() => { alert('模拟：已生成海报保存到相册，扫码即可加入'); setShowInviteModal(false); }}
